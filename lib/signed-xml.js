...
function findAncestorNs(doc, docSubsetXpath, namespaceResolver) {
  var docSubset = xpath.selectWithResolver(
    docSubsetXpath,
    doc,
    namespaceResolver
  );
...
SignedXml.prototype.getCanonReferenceXml = function (doc, ref, node) {
  ...
  if (Array.isArray(ref.transforms)) {
    ref.ancestorNamespaces = findAncestorNs(
      doc,
      ref.xpath,
...
SignedXml.prototype.addReference = function (
  ...
  console.log(
    "adding reference inclusiveNamespacesPrefixList",
    inclusiveNamespacesPrefixList
  );
...
SignedXml.prototype.createReferences = function (doc, prefix) {
  ...
  for (var n in this.references) {
    ...
    for (var h in nodes) {
      ...
      // if inclusive namespaces have been specified add them here:
      if (
        ref.inclusiveNamespacesPrefixList &&
        ref.inclusiveNamespacesPrefixList.length > 0
      ) {
        var prefixList = ref.inclusiveNamespacesPrefixList.join(" ");
        res +=
          '<ec:InclusiveNamespaces xmlns:ec="http://www.w3.org/2001/10/xml-exc-c14n#" PrefixList="' +
          prefixList +
          '" />';
        console.log("xml crypto added InclusiveNamespaces");
      }
